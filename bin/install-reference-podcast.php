...
$directory = $root . '/plugins/podcast';
$json = file_get_contents($directory . '/plugin.json');
if (!is_string($json)) {
    throw new RuntimeException('The bundled Podcast manifest is missing.');
}
...
$installed = $registry->find('podcast');
if ($installed === null) {
...
try {
    ...
    if ($installed === null) {
        $registry->remove('podcast');
    }
    throw $exception;
}
$options = getopt('', ['enable']);
if (array_key_exists('enable', $options)) {
    $registry->setState('podcast', 'enabled');
}
$state = array_key_exists('enable', $options) ? ' and enabled' : '';
fwrite(STDOUT, 'Podcast reference plugin is installed' . $state . ".\n");
