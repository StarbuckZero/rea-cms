...
$directory = $root . '/plugins/events';
$json = file_get_contents($directory . '/plugin.json');
if (!is_string($json)) {
    throw new RuntimeException('The bundled Events / Schedule manifest is missing.');
}
...
$installed = $registry->find('events');
if ($installed === null) {
...
if (array_key_exists('enable', $options)) {
    $registry->setState('events', 'enabled');
}
$state = array_key_exists('enable', $options) ? ' and enabled' : '';
fwrite(STDOUT, 'Events / Schedule reference plugin is installed' . $state . ".\n");
