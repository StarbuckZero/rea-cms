...
$directory = $root . '/plugins/blog';
$json = file_get_contents($directory . '/plugin.json');
if (!is_string($json)) {
    throw new RuntimeException('The bundled Blog manifest is missing.');
}
...
if (array_key_exists('enable', $options)) {
    $registry->setState('blog', 'enabled');
}
$state = array_key_exists('enable', $options) ? ' and enabled' : '';
fwrite(STDOUT, 'Blog reference plugin is installed' . $state . ".\n");
