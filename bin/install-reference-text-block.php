...
$directory = $root . '/plugins/text_block';
$json = file_get_contents($directory . '/plugin.json');
if (!is_string($json)) {
    throw new RuntimeException('The bundled Text Block manifest is missing.');
}
...
$installed = $registry->find('text_block');
if ($installed === null) {
...
try {
    ...
    if ($installed === null) {
        $registry->remove('text_block');
    }
    throw $exception;
}
$options = getopt('', ['enable']);
if (array_key_exists('enable', $options)) {
    $registry->setState('text_block', 'enabled');
}
$state = array_key_exists('enable', $options) ? ' and enabled' : '';
fwrite(STDOUT, 'Text Block reference plugin is installed' . $state . ".\n");
